...
class GopherApplication:
    ...
    def _status_callback(self, message, level="ok"):
    ...
    def _handle_focus_change(self, focus_part):
    ...
    def navigate_forward(self, line):
        """Navigate to a new location"""
        try:
            walkable = line.type not in ["bin", "txt", "hex", "img", "gif", "png", "rtf", "pdf", "xml"]
    ...
    def open_http_link(self, line, offset=0):
        """Open HTTP link in external application"""
        url = line.location.url.replace("URL:", "")
        
        if self.content_window.image_preview and INLINE_IMAGES_ENABLED and is_image(url):
    ...
    def open_file(self, line):
        ...
        filename = f"{os.path.expanduser('~')}/Downloads/{line.location.url.rsplit('/')[-1]}"
    ...
    def show_search_overlay(self, line):
        """Show search overlay"""
        widget = urwid.Filler(
            urwid.AttrMap(SearchOverlay(
                self, line, 
    ...
    def show_download_overlay(self, location):
        """Show download overlay"""
        widget = urwid.Filler(
            urwid.AttrMap(DownloadOverlay(
                self, location,
    ...
    def show_exit_overlay(self):
        ...
        widget = urwid.Filler(urwid.AttrMap(ExitOverlay(
            self,
    ...
    def show_bookmarks(self):
        ...
        lines = [self.client._parse_line(line, self.history.current_location) for line in content]
    ...
    def show_history(self):
        ...
        lines = [self.client._parse_line(line, self.history.current_location) for line in content[::-1]]
        self.client.current_location_map = lines
    ...
    def _handle_search(self, line, query):
    ...
    def _handle_bookmark_save(self, bookmark_name):
    ...
    def _handle_download(self, location, file_path):
    ...
    def run(self):
        ...
        try:
            ...
            if self.content_window.sound_preview_thread:
                os.killpg(os.getpgid(self.content_window.sound_preview_thread.pid), signal.SIGTERM)
