...
class Location:
    def __init__(
        self, host, port, url, focus=0, walkable=True, bookmarks=False, history=False
    ):
        self.host = host
    ...
    def keypress(self, size, key):
        ...
        if INLINE_IMAGES_ENABLED and self.image_preview:
            ...
            if key in ["d"]:
                widget = urwid.Filler(
                    urwid.AttrMap(
                        DownloadOverlay(self.gopher, location), "download_overlay"
                    )
                )
...
class Gopher:
    ...
    def download_http(self, url, file_path=None):
        ...
        self.status_bar.set_status(f"downloading: {url}", level="loading")
    ...
    def download(self, location, file_path=None):
        ...
        self.status_bar.set_status(
            f"downloading: gopher://{location.host}{location.url}", level="loading"
        )
